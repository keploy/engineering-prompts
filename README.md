...
- **Refactor Code for Better Readability**
  ...
  ```text
  "Your task is to review the following codebase with the goal of enhancing its readability and maintainability. Focus on identifying complex logic, unclear naming conventions, and any redundant or repetitive code patterns. Refactor where necessary to simplify functions, use descriptive and consistent naming, and streamline the structure without changing functionality. Once refactored, return the updated code along with a concise summary explaining the key changes made and how they contribute to better understanding and future maintenance."
  ```

- **Translate Code from One Language to Another**

  ```text
  "Convert the provided code from {source_language} to {target_language}, ensuring the original functionality is fully preserved. Modify any language specific features to reflect the natural style and conventions of the target language. After translation, briefly explain any significant syntax or structural differences encountered and how they were addressed."
  ```

- **Create Documentation from Code**

  ```text
  "Analyze the provided code and extract the essential details needed to produce comprehensive documentation. Include clear explanations for functions, methods, classes, and the overall purpose of the code. Return the documentation in well-structured **markdown format.**"
  ```

- **Provide Code Explanations**

  ```text
  "You must review the provided code and break down its functionality step-by-step. Clearly explain the purpose of each major section, why it’s necessary, and how it contributes to the overall logic. Keep the explanation concise, focused, and easy to follow."
  ```

- **Generate Git Commit Messages**

  ```text
  "Review the code changes and create clear, concise commit messages that accurately describe what was changed and why. Follow the Conventional Commits standard to ensure consistency and readability."
  ```

- **Debug Code**

  ```text
  "Examine the provided code alongside the error message. Identify the root cause of the issue, suggest appropriate fixes, and return the corrected code. Include brief explanations for each change made and how it resolves the error."
  ```

- **Write Regular Expressions**

  ```text
  "Create a regular expression to handle the following use case: {use_case}. For example, if the task is to validate email addresses, produce a regex pattern that accurately matches valid email formats. Return the regular expression and provide a brief explanation of how it works and what each part does."
  ```

- **Generate Boilerplate Code**

  ```text
  "Create boilerplate code for a {project_type} using {programming_language}. The code should include a basic setup, essential functions, and a clear structure to serve as a starting point. Return the complete code along with brief instructions on how to customize or extend it for specific needs."
  ```

- **Automate Code Formatting**

  ```text
  "Configure an automatic code formatting system using {tool}, ensuring it runs on each commit using pre-commit hooks or an equivalent method. Provide the required configuration files and setup steps, and explain how this integration fits into the development workflow."
  ```

- **Explain Data Structures and Algorithms**

  ```text
  "Provide a clear explanation of {data_structure/algorithm}, including its core concept, typical use cases, and time complexity for key operations. Include common implementations and provide code examples where applicable to illustrate how it works in practice."
  ```

- **Generate Mock Data**

  ```text
  "Generate realistic mock data based on the following schema: {data_schema}. Include a diverse range of values suitable for testing scenarios. Return the data in the requested format (e.g., JSON, CSV), and ensure it reflects the structure and data types defined in the schema."
  ```

- **Create Code for a New Feature**

  ```text
  "Develop code to implement the following feature: {feature_description}. Ensure the solution is fully functional and integrates cleanly with the existing codebase. Return the complete implementation along with a brief explanation of how it works and how it fits into the overall system."
  ```

- **Suggest Performance Optimizations**

  ```text
  "Analyze the provided code to identify potential performance bottlenecks. Recommend and implement optimizations aimed at improving time complexity, reducing memory usage, or enhancing algorithm efficiency. Return the optimized code along with clear explanations of the changes and their impact on performance."
  ```

- **Generate Build Scripts**

  ```text
  "Develop a build script to automate the build process for a {project_type} using {build_tool}. The script should cover key steps such as compiling source code, packaging artifacts, and handling versioning. Return the complete build script along with clear setup instructions and an overview of how it fits into the project workflow."
  ```

- **Offer Security Best Practices**

  ```text
  "Provide a comprehensive list of security best practices for a {application_type}. Focus on key areas including authentication, authorization, data protection, and secure coding. Offer actionable recommendations with examples where appropriate to illustrate effective implementation."
  ```

- **Assist in Code Reviews**

  ```text
  "Evaluate the provided code and offer constructive feedback focused on readability, performance, security, and maintainability. Suggest specific improvements and explain the reasoning behind each recommendation to help enhance overall code quality."
  ```

- **Test for Compatibility After Upgrading Node.js Version**

  ```text
  """After upgrading Node.js from version 14 to 18, use Keploy to run automated integration tests to ensure the application continues to function as expected. Set up Keploy to test key components—backend, APIs, and frontend integrations—with a focus on:
  --API response accuracy
  --Performance under the new Node.js version
  --Compatibility with external dependencies that may be impacted
  ```

Configure Keploy to compare current results against previously recorded behavior under Node.js 14, in order to detect regressions or functional discrepancies introduced by the upgrade.

Deliverables:
--Keploy configuration for test setup
--Defined test cases covering critical workflows
--A report of the test results, highlighting any changes, regressions, or compatibility issues"""
...
```text
"Integrate {third_party_service} into the existing project. Provide clear, step-by-step instructions for setup, including authentication, configuration, and API interaction. Return the full integration code along with explanations of how it works and how it connects to the rest of the application."
````

- **Write a Dockerfile for the Application**

  ```text
  "Write a Dockerfile to containerize the provided application. Ensure it sets up the environment correctly, installs all necessary dependencies, and exposes the appropriate ports for the application to run. Return the complete Dockerfile along with a brief explanation of each instruction to clarify its role in the containerization process."
  ```

- **Provide UX/UI Design Advice**

  ```text
  "Analyze the provided {website/app} and offer targeted UX/UI design recommendations. Focus on enhancing usability, accessibility, and visual aesthetics. Return a clear list of specific design improvements, supported by practical examples or design principles where applicable."
  ```

- **Suggest Testing Strategies**

  ```text
  "Propose a comprehensive testing strategy for a {project_type}. The strategy should cover unit tests, integration tests, and end-to-end tests, with clear goals for each layer. Recommend suitable testing frameworks and tools, and include code examples or use cases to illustrate their application."
  ```

- **Test for Regressions After Refactoring Code with Keploy**

  ```text
  "After refactoring the codebase, use Keploy to run automated tests and verify that no regressions have been introduced. Configure Keploy to validate that all existing functionalities remain intact by:
  --Running integration tests that cover API endpoints, data processing, and user interactions
  --Comparing the current behavior of the refactored code against previously recorded test cases
  --Detecting any functional discrepancies or regressions introduced during refactoring
  ```

Return the following:

--Keploy test configuration
--Defined test cases covering critical workflows
--Test results that demonstrate whether the refactored code maintains expected behavior"
